...
class PairLoader(Dataset):
	def __init__(self, data_dir, sub_dir, mode, size=256, edge_decay=0, only_h_flip=False):
		...
		self.img_names = self.get_image_file_names(os.path.join(self.root_dir, 'lr'))
	...
	def __getitem__(self, idx):
		...
		if os.path.splitext(img_name)[1].lower() in valid_image_extensions:
			source_img = read_img(os.path.join(self.root_dir, 'lrd', img_name)) * 2 - 1
			target_img = read_img(os.path.join(self.root_dir, 'lr', img_name)) * 2 - 1
